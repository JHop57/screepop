...
class Prioritizer {
    ...
    private avePercentile: number = 0.7; //not exact, actual is higher. close 'nuff.
    ...
    private execute(todo: Todo): number {
        ...
        let newScore = todo.func();
        let cpuAfter = Game.cpu.getUsed();
        let newCpuCost = cpuAfter - cpuBefore;
    ...
    private refineInterval(){
        let sum = _.sum(this.scheduled, s => s.score/s.cpuCost);
        let ave = sum/this.scheduled.length;

        for(let todo of this.scheduled){
            let difference = ave - (todo.score/todo.cpuCost);
    ...
    public run() {
        ...
        let scoring = (s: Todo) => (s.score*s.priority*Math.pow((Game.time-s.lastExecuted), 2)) / (s.cpuCost*Math.pow(s.interval, 2));
    ...
    public requestSpecific(name: string, insistance: number = 1): boolean {
        let todo = this.scheduled.find(s => s.name == name);
        if(!todo) {
    ...
    public readMem(){
        let mem = Memory.prioritizer;
        if(!mem) return;
        this.cpuLimit = mem.cpuLimit;
        for(let memTodo of mem.scheduled){
            let todo = this.scheduled.find(s => s.name == memTodo.name);
            if(!todo) continue;
